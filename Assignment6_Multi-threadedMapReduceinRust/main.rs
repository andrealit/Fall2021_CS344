...
fn main() {
    ...
    let xs1 = xs.clone();
    let thread1 = thread::spawn(move || map_data(&xs1[0]));
    ...
    // from 0 to number of partitions
    for i in 0..num_partitions {
        let pdata_clone = pdata.clone();
        // spawn a thread with a move closure 
        handles.push(thread::spawn(move || map_data(&pdata_clone[i])));
    }

    // 4. Collects the intermediate sums from all the threads
    let mut intermediate_sums_pd : Vec<usize> = Vec::new();
    for handle in handles {
        // join threads, unwrap them, and push onto intermediate sums
        let temp = handle.join().unwrap();
        intermediate_sums_pd.push(temp);
    }

    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", intermediate_sums_pd);
    // 5. Calls reduce_data to process the intermediate sums
    let sum_reduce = reduce_data(&intermediate_sums_pd);
    // 6. Prints the final sum computed by reduce_data
    println!("Sum = {}", sum_reduce);
}
...
fn partition_data(num_partitions: &usize, v: &Vec<usize>) -> Vec<Vec<usize>> {
    let partition_size = v.len() / num_partitions; // dividing by length
    let remainder = v.len() % num_partitions; // find the remainder
    let mut pdata: Vec<Vec<usize>> = Vec::new(); // a vector that holds the vectors
    ...
    // check for multiples
    while start != v.len() {
        let mut temp : Vec<usize> = Vec::new(); // created a new vector
        for i in start..end {
            temp.push(v[i]); // place the vector into the temp
        }
        pdata.push(temp); // fill the partition
        start = end; // go to the next

        // without a remainder, see if the end is equal to the length of the vector
        if remainder == 0 {
            if end == v.len() {
                break;
            }
        } else {
            if end == v.len() - remainder {
                break;
            }
        }
    ...
    // else if not a multiple, must add the integers to the partitions
    if remainder != 0 {
        let mut idx = 0;
        for i in end..v.len() {
            pdata[idx].push(v[i]);
    ...
    // Return a vector that contains the vectors of integers
    pdata
}
